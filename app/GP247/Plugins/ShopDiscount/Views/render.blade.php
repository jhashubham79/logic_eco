<div class="row">
    <div class="form-group col-md-12">
      <label class="control-label" for="inputGroupSuccess3"><i class="fa fa-exchange" aria-hidden="true"></i> {{ gp247_language_render('cart.coupon') }}
          <span style="display:inline; cursor: pointer; display: {{ (!empty(session('totalMethod')[$plugin['key']]))?'inline':'none' }}" class="text-danger" id="removeCoupon">({{ gp247_language_render('cart.remove_coupon') }} <i class="fa fa fa-times"></i>)</span>
      </label>
      <div id="coupon-group" class="input-group {{ Session::has('error_discount')?'has-error':'' }}">
        ...
        <span class="input-group-text {{ ($plugin['permission'])?'':'disabled' }}"  {!! ($plugin['permission'])?'id="coupon-button"':'' !!} style="cursor: pointer;" data-loading-text="<i class='fa fa-spinner fa-spin'></i> checking">{{ gp247_language_render('cart.apply') }}</span>
        </div>
      </div>
