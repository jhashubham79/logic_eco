...
<div class="aside-item col-sm-6 col-lg-12">
    <h6 class="aside-title">{{ gp247_language_render('front.products_last_view') }}</h6>
    <!--last_view_product-->
            ...
            <article class="post post-minimal">
              <div class="unit unit-spacing-sm flex-column flex-lg-row align-items-lg-center">
                <div class="unit-left"><a class="post-minimal-figure" href="{{ $productLastView->getUrl() }}">
                    <img src="{{ gp247_file($productLastView->getThumb()) }}" alt="" width="106" height="104"></a></div>
                <div class="unit-body">
                  <p class="post-minimal-title"><a href="{{ $productLastView->getUrl() }}">{{ $productLastView->name}}</a></p>
                  <div class="post-minimal-time">
                    <time datetime="{{ gp247_datetime_to_date($productLastView['timelastview'],'Y-m-d H:i:s') }}">{{ gp247_datetime_to_date($productLastView['timelastview'],'Y-m-d H:i:s') }}</time>
                  </div>
                </div>
              </div>
            </article>
