    ...
    <div class="col-md-12">
       <div class="card">
          ...
          <div class="card-header with-border">
              <h3 class="card-title">{{ gp247_language_render('order.order_detail') }} #{{ $order->id }}</h3>
              <div class="card-tools not-print">
                  <div class="btn-group float-right" style="margin-right: 0px">
                      <a href="{{ gp247_route_admin('admin_order.index') }}" class="btn btn-flat btn-default"><i class="fa fa-list"></i>&nbsp;{{ gp247_language_render('admin.back_list') }}</a>
                  </div>
                  <div class="btn-group float-right" style="margin-right: 10px;border:1px solid #c5b5b5;">
                      <a class="btn btn-flat" target=_new title="Invoice" href="{{ gp247_route_admin('admin_order.invoice', ['order_id' => $order->id]) }}"><i class="far fa-file-pdf"></i><span class="hidden-xs"> {{ gp247_language_render('order.invoice') }}</span></a>
                  </div>
              </div>
          </div>
          ...
          <div class="row" id="order-body">
            <div class="col-sm-6">
                 <table class="table table-hover box-body text-wrap table-bordered">
                    ...
                    <tr>
                      <td class="td-title">{{ gp247_language_render('order.email') }}:</td><td>{!! empty($order->email)?'N/A':$order->email!!}</td>
                    </tr>
            ...
            <div class="col-sm-6">
                <table  class="table table-bordered">
                 ...
                 <table class="table table-hover box-body text-wrap table-bordered">
                    <tr>
                      <td class="td-title"><i class="far fa-money-bill-alt nav-icon"></i> {{ gp247_language_render('order.currency') }}:</td><td>{{ $order->currency }}</td>
                    </tr>
                    <tr>
                      <td class="td-title"><i class="fas fa-chart-line"></i> {{ gp247_language_render('order.exchange_rate') }}:</td><td>{{ ($order->exchange_rate)??1 }}</td>
                    </tr>
                </table>
            </div>
            ...
            <table class="table table-hover box-body text-wrap table-bordered">
                <thead>
                  <tr>
                    ...
                    <th>{{ gp247_language_render('product.sku') }}</th>
                    <th class="product_price">{{ gp247_language_render('product.price') }}</th>
                    <th class="product_qty">{{ gp247_language_render('product.quantity') }}</th>
                    <th class="product_total">{{ gp247_language_render('product.total_price') }}</th>
                    <th class="product_tax">{{ gp247_language_render('product.tax') }}</th>
                    <th>{{ gp247_language_render('action.title') }}</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr  id="add-item" class="not-print">
                      <td colspan="7">
                        <button  type="button" class="btn btn-flat btn-success" id="add-item-button"  title="{{gp247_language_render('action.add') }}"><i class="fa fa-plus"></i> {{ gp247_language_render('action.add') }}</button>
                        &nbsp;&nbsp;&nbsp;<button style="display: none; margin-right: 50px" type="button" class="btn btn-flat btn-warning" id="add-item-button-save"  title="Save"><i class="fa fa-save"></i> {{ gp247_language_render('action.save') }}</button>
                    </td>
                  </tr>
                </tbody>
              </table>
                ...
                <table   class="table table-bordered">
                  @foreach ($dataTotal as $element)
                    @if ($element['code'] =='subtotal')
                      <tr><td  class="td-title-normal">{!! $element['title'] !!}:</td><td style="text-align:right" class="data-{{ $element['code'] }}">{{ gp247_currency_format($element['value']) }}</td></tr>
                    @endif
                    @if ($element['code'] =='tax')
                    <tr><td  class="td-title-normal">{!! $element['title'] !!}:</td><td style="text-align:right" class="data-{{ $element['code'] }}">{{ gp247_currency_format($element['value']) }}</td></tr>
                    @endif
                  ...
                  @endforeach
                    ...
                    <tr  {!! $style !!}  class="data-balance"><td>{{ gp247_language_render('order.totals.balance') }}:</td><td style="text-align:right">{{($order->balance === NULL)?gp247_currency_format($order->total):gp247_currency_format($order->balance) }}</td></tr>
              </table>
            </div>
          ...
          <div class="col-sm-6">
            ...
            <div class="card collapsed-card"">
              <div class="card-header border-transparent">
                <h3 class="card-title">{{ gp247_language_render('admin.order.order_history') }}</h3>
              ...
              <div class="card-body p-0 out">
                <div class="table-responsive">
                  ...
                  <table  class="table m-0" id="history">
                    <tr>
                      <th>{{ gp247_language_render('admin.order.history_staff') }}</th>
                      <th>{{ gp247_language_render('admin.order.history_content') }}</th>
                      <th>{{ gp247_language_render('admin.order.history_time') }}</th>
                    </tr>
                  @foreach ($order->history->sortKeysDesc()->all() as $history)
                    <tr>
                      <td>{{ \GP247\Core\Models\AdminUser::find($history['admin_id'])->name??'' }}</td>
