      ...
      <div class="card-header">
        ...
        @if ($layout == 'edit')
          <div class="btn-group float-right" style="margin-right: 5px">
              <a href="{{ gp247_route_admin('admin_order_status.index') }}" class="btn  btn-flat btn-default" title="List"><i class="fa fa-list"></i><span class="hidden-xs"> {{ gp247_language_render('admin.back_list') }}</span></a>
          </div>
        @endif
      </div>
      <!-- /.card-header -->
      ...
      <form action="{{ $url_action }}" method="post" accept-charset="UTF-8" class="form-horizontal" id="form-main">
        <div class="card-body">
          <div class="form-group row {{ $errors->has('name') ? ' text-red' : '' }}">
            <label for="name" class="col-sm-2 col-form-label">{{ gp247_language_render('admin.order_status.name') }}</label>
            <div class="col-sm-10 ">
            ...
            $.ajax({
                ...
                success: function (data) {
                    if(data.error == 1){
                      ...
                      window.location.replace('{{ gp247_route_admin('admin_order_status.index') }}');
                    }
