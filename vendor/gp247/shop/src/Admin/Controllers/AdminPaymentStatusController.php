...
class AdminPaymentStatusController extends RootAdminController
{
    ...
    {
        $data = [
            'title' => gp247_language_render('admin.payment_status.list'),
            'title_action' => '<i class="fa fa-plus" aria-hidden="true"></i> ' . gp247_language_render('admin.payment_status.add_new_title'),
            'subTitle' => '',
            'icon' => 'fa fa-indent',
            'urlDeleteItem' => gp247_route_admin('admin_payment_status.delete'),
            'removeList' => 0, // 1 - Enable function delete list item
            'buttonRefresh' => 0, // 1 - Enable button refresh
            'url_action' => gp247_route_admin('admin_payment_status.post_create'),
        ];

        $listTh = [
            'id' => 'ID',
            'name' => gp247_language_render('admin.payment_status.name'),
        ...
        foreach ($dataTmp as $key => $row) {
            ...
            $arrAction = [
                '<a href="' . gp247_route_admin('admin_payment_status.edit', ['id' => $row['id'] ? $row['id'] : 'not-found-id']) . '"  class="dropdown-item"><i class="fa fa-edit"></i> '.gp247_language_render('action.edit').'</a>',
                ];
        ...
        $data['layout'] = 'index';
        return view('gp247-shop-admin::screen.payment_status')
            ->with($data);
    }
    ...
    public function postCreate()
    {
        ...
        return redirect()->route('admin_payment_status.index')->with('success', gp247_language_render('action.create_success'));
    }
    ...
    public function edit($id)
    {
        ...
        'url_action' => gp247_route_admin('admin_payment_status.edit', ['id' => $payment_status['id']]),
        ...
        'id' => 'ID',
        'name' => gp247_language_render('admin.payment_status.name'),
        'action' => gp247_language_render('admin.payment_status.action.title'),
    ];
        $obj = new ShopPaymentStatus;
        $obj = $obj->orderBy('id', 'desc');
        $dataTmp = $obj->paginate(20);
        ...
        if (!request()->ajax()) {
            ...
            foreach ($arrID as $key => $row) {
                if (isset($this->statusProtected()[$row])) {
                    return response()->json(['error' => 1, 'msg' => gp247_language_render('admin.payment_status.status_protected', ['status' => $this->statusProtected()[$row]])]);
                }
            }
